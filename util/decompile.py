...
def decompile_worker(src_file: str, dest_path: str):
    # Remove any existing output from a previous run
...
def _prepare_zip(src_dir: str, zip_name: str, dst_dir: str):
    """
    Extract a zip to a temp directory, clean up stale files from dest, and
    collect files to decompile. Returns (tmp_dir, to_decompile list).
    """
    file_stem = get_file_stem(zip_name)
    src_zip = os.path.join(src_dir, zip_name)
    dest_dir = os.path.join(dst_dir, file_stem)
    ...
    for root, dirs, files in os.walk(tmp_dir.name):
        for filename in fnmatch.filter(files, python_compiled_ext):
            ...
            src_file_rel_path = get_rel_path(src_file, tmp_dir.name)
    ...
    # Remove stale files and orphaned temp files from dest
    stale_files = []
        ...
        for root, dirs, files in os.walk(dest_dir):
            for filename in files:
                ...
                if not filename.endswith(".py"):
                    os.remove(dest_file)
                    stale_files.append(dest_rel_path)
                else:
                    src_rel_path = str(Path(dest_rel_path).with_suffix(".pyc"))
                    if src_rel_path not in src_rel_paths:
                        os.remove(dest_file)
                        stale_files.append(dest_rel_path)
    if stale_files:
        print(f"Removed {len(stale_files)} stale file(s) from {dest_dir}:")
        for f in sorted(stale_files):
            print(f"  - {f}")

    return tmp_dir, to_decompile


def decompile_zips(src_dirs, dst_dir: str) -> None:
    """
    Decompiles zip files from one or more source directories.
    All zips are extracted and cleaned up first, then decompiled in a single pool.

    :param src_dirs: Directory or list of directories to search for zip files
        ...
        for root, dirs, files in os.walk(src_dir):
            for ext_filter in script_package_types:
                for filename in fnmatch.filter(files, ext_filter):
                    ...
                    tmp_dir, to_decompile = _prepare_zip(root, filename, dst_dir)
                    tmp_dirs.append(tmp_dir)
                    all_to_decompile.extend(to_decompile)

    if not all_to_decompile:
        return
